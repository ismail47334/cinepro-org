...
export class SmashyStreamBollywood extends BaseProvider {
    ...
    private async getSources(media: ProviderMediaObject): Promise<ProviderResult> {
        try {
            const url = media.type==='movie' ? `${this.BASE_URL}/playere.php?tmdb=${media.tmdbId}` : `${this.BASE_URL}/playere.php?tmdb=${media.tmdbId}&season=${media.s}&episode=${media.e}`;
            const resp = await fetch(url, { headers: this.HEADERS as any });
            if(!resp.ok) return this.emptyResult(`HTTP ${resp.status}`, media);
            const html = await resp.text();
            const sources: Source[] = [];
            const patterns = [/file:\s*["'](https:\/\/[^"']+\.m3u8[^"']*)["']/gi, /src:\s*["'](https:\/\/[^"']+\.(?:m3u8|mp4)[^"']*)["']/gi];
            for(const pat of patterns){
                let m; while((m=pat.exec(html))!==null){
                    sources.push({ url: this.createProxyUrl(m[1], this.HEADERS as any), type: (m[1].includes('.m3u8')?'hls':'mp4') as SourceType, quality: '1080', audioTracks: [{language:'eng', label:'English'}], provider: {id:this.id, name:this.name} });
                } if(sources.length) break;
            }
            if(sources.length===0){
                sources.push({ url: url, type: 'hls' as SourceType, quality: 'auto', audioTracks: [{language:'eng', label:'English'}], provider: {id:this.id, name:this.name} });
            }
            return { sources, subtitles: [], diagnostics: [] };
        } catch(e){ return this.emptyResult(e instanceof Error ? e.message : 'Failed', media); }
    }
    private emptyResult(msg:string, media:ProviderMediaObject): ProviderResult {
        return { sources: [], subtitles: [], diagnostics: [{ code:'PROVIDER_ERROR', message:`${this.name}: ${msg}`, field:'', severity:'error'} as Diagnostic] };
    }
}
